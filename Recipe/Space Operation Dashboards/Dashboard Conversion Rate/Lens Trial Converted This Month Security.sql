...
q_trial_this_month = filter q_membership by 
	...
	&& {{row(Reserva_Checkin_Time_1.selection, [0], [\"min\", \"max\"]).asDateRange(\"date('Reserva.Checkin_Time__c_Year', 'Reserva.Checkin_Time__c_Month', 'Reserva.Checkin_Time__c_Day')\")}}
	;
...
q_trial_converted_this_month = filter q_conversion by 'Last_Purchase_Membership__c' is not null
	...
	&& {{coalesce(row(Reserva_Checkin_Time_1.selection, [0], [\"min\", \"max\"]), [\"current month\" , \"current month\"]).asDateRange(\"date('Databas.Reserva.Checkin_Time__c_Year', 'Databas.Reserva.Checkin_Time__c_Month', 'Databas.Reserva.Checkin_Time__c_Day')\")}}
	&& {{coalesce(row(Order_Create_Date__c_1.selection, [0], [\"min\", \"max\"]), [\"current month\" , \"current month\"]).asDateRange(\"date('Order_Create_Date__c_Year', 'Order_Create_Date__c_Month', 'Order_Create_Date__c_Day')\")}}
	;
